 ...
 *
 * Three audiences:
 *   * Tenant super-admin CRUD — ``/api/tenant-provider-plans`` (this
 *     file's primary surface).
 *   * Anonymous signup picker — public read at
 *     ``/api/tenant-provider-plans/public/<vertical>``. Used by the
 *     in-tenant doctor/clinic/hospital signup forms.
 *   * Provider's own "what plan am I on?" — ``/api/tenant-provider-plans/me``.
 */
import { apiSlice } from '../../../app/api/apiSlice';


const BASE = '/api/tenant-provider-plans';
...
const tenantProviderPlanEndpoints = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        // Mirrors ``/api/platform/feature-paths`` but accepts tenant
        ...
        listTenantProviderSubscriptions: builder.query({
            query: ({ vertical, status } = {}) => ({
                url: '/api/tenant-provider-subscriptions',
                method: 'GET',
                params: {
                    ...(vertical ? { vertical } : {}),
                    ...(status ? { status } : {}),
        ...
        changeTenantProviderSubscriptionPlan: builder.mutation({
            query: ({ id, plan_id }) => ({
                url: `/api/tenant-provider-subscriptions/${id}`,
                method: 'PATCH',
                data: { plan_id },
            }),
            invalidatesTags: (result, error, arg) => [
        ...
        activateTenantProviderSubscription: builder.mutation({
            query: (id) => ({
                url: `/api/tenant-provider-subscriptions/${id}/activate`,
                method: 'POST',
            }),
            invalidatesTags: (result, error, id) => [
                { type: 'TenantProviderSubscription', id: 'LIST' },
        ...
        listUnsubscribedProviders: builder.query({
            query: (vertical) => ({
                url: '/api/tenant-provider-subscriptions/unsubscribed-providers',
                method: 'GET',
                params: { vertical },
            }),
        ...
        approveTenantProviderSubscriptionRequest: builder.mutation({
            query: (id) => ({
                url: `/api/tenant-provider-subscriptions/${id}/approve-request`,
                method: 'POST',
            }),
            invalidatesTags: (result, error, id) => [
                { type: 'TenantProviderSubscription', id: 'LIST' },
