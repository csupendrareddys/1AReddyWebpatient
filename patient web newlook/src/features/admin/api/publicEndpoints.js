 ...
 * signup. These hit ``/api/public/*`` on the backend which runs without a
...
const publicEndpoints = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        listPublicPlanTypes: builder.query({
            query: () => ({ url: `${PUBLIC_BASE}/plan-types`, method: 'GET' }),
            transformResponse: (response) => response?.data || [],
            providesTags: [{ type: 'PlanType', id: 'PUBLIC_LIST' }],
        }),
        ...
        listPublicTenantProviderPlans: builder.query({
            query: (vertical) => ({
                url: `/api/tenant-provider-plans/public/${encodeURIComponent(vertical)}`,
                method: 'GET',
            }),
