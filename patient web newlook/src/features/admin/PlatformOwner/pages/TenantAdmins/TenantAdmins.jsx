...
const TenantAdmins = () => {
    ...
    const [setPlan, planState] = useSetTenantPlanMutation();
    ...
    const handlePlanChange = async (newPlan) => {
        try {
            await setPlan({ tenantId, plan: newPlan }).unwrap();
            setSnack({ open: true, severity: 'success',
                message: `Plan updated to "${newPlan}".` });
        } catch (err) {
            setSnack({ open: true, severity: 'error',
                message: err?.data?.error || 'Failed to update plan.' });
        }
    };
                ...
                <Stack direction="row" alignItems="flex-start" spacing={2}>
                    ...
                    <FormControl size="small" sx={{ minWidth: 200 }}>
                        ...
                        <Select
                            ...
                            disabled={planState.isLoading || !tenant}
                        >
                            {PLAN_OPTIONS.map((p) => (
