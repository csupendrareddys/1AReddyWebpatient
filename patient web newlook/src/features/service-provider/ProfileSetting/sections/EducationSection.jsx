...
const EducationSection = React.memo(({ previewMode = false, configOverride = null, registerSave }) => {
    const {
        educationState,
        ...
        handleSaveEducation,
    } = useEducationDetails(previewMode);
    ...
    const getFieldApprovalStatus = (fieldName) => {
        const key = `education.${fieldName}`;
        return fieldStatuses[key]?.status || null;
    };

    const pendingFields = Object.entries(fieldStatuses)
        .filter(([key, info]) => key.startsWith('education.') && (info.status === 'pending' || info.status === 'query'))
        ...
        <Box>
            {pendingFields.length > 0 && !previewMode && (
                <Alert severity="warning" sx={{ mb: 2 }}>
                    <Typography variant="body2" fontWeight="bold">
                        {pendingFields.length} education field(s) waiting for approval
                    </Typography>
                </Alert>
            )}
            ...
            {cfg.isSectionVisible('education_graduation') && <Paper sx={{ p: 3, mb: 3 }}>
                <Box display="flex" alignItems="center" gap={1} mb={2}>
                    ...
                    <ApprovalChip status={getFieldApprovalStatus('graduation_degree')} />
                </Box>
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <FormControl fullWidth>
                            ...
                            >
                                {/* No hardcoded ``MBBS / BDS / BAMS``
                                    fallback — that misled operators in
                                    the editor preview into thinking the
                                    dropdown was populated when the
                                    master_degrees table was actually
                                    empty. Honest empty-state instead. */}
                                {mergeSaved(educationState.dropdownOptions?.degrees, educationState.graduation.degree).length > 0
                                    ...
                                    : <MenuItem disabled value="">(no degrees configured — add via Master Data)</MenuItem>
                                }
                            </Select>
                        </FormControl>
                    </Grid>
                        ...
                        <FormControl fullWidth>
                            ...
                            >
                                {mergeSaved(educationState.dropdownOptions?.ugSpecializations, educationState.graduation.specialization).length > 0
                                    ...
                                    : <MenuItem disabled value="">(no specializations configured)</MenuItem>
                                }
                            </Select>
                        </FormControl>
                    </Grid>
                        ...
                        <FormControl fullWidth>
                            ...
                            >
                                {mergeSaved(educationState.dropdownOptions?.states, educationState.graduation.state).length > 0
                                    ...
                                    : <MenuItem disabled value="">(no states configured)</MenuItem>
                                }
                            </Select>
                        </FormControl>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                        <FormControl fullWidth>
                            ...
                            >
                                {/* Defensive: ``evaluationCriteria`` was unguarded
                                    while the sibling dropdowns (degrees /
                                    states / etc.) used a length > 0 check.
                                    Preview mode mounts this component with
                                    a configOverride that doesn't hydrate
                                    the redux slice, so dropdownOptions
                                    came back partial and ``.map`` of
                                    undefined crashed the entire preview
                                    tab (TypeError: Cannot read properties
            ...
            {cfg.isSectionVisible('education_post_graduation') && <Paper sx={{ p: 3, mb: 3 }}>
                <Box display="flex" alignItems="center" gap={1} mb={2}>
                    ...
                    <ApprovalChip status={getFieldApprovalStatus('post_graduation_degree')} />
                </Box>
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <FormControl fullWidth>
            ...
            {cfg.isSectionVisible('education_super_speciality') && <Paper sx={{ p: 3, mb: 3 }}>
                <Box display="flex" alignItems="center" gap={1} mb={2}>
                    ...
                    <ApprovalChip status={getFieldApprovalStatus('super_speciality_degree')} />
                </Box>
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <FormControl fullWidth>
            ...
            {cfg.isSectionVisible('education_other_certification') && <Paper sx={{ p: 3, mb: 3 }}>
                <Box display="flex" alignItems="center" gap={1} mb={2}>
                    ...
                    <ApprovalChip status={getFieldApprovalStatus('other_certification_degree')} />
                </Box>
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <TextField
