 ...
 *
 * Backend counterpart: ``/api/admin/operations/<vertical>-members/<id>/act/…``
 ...
 * facility as a path parameter on ``/api/admin/clinics/<id>`` style routes and
...
/**
 * Build the URL for ``path`` (leading slash, relative to ``/api``) in the given
 * scope. Unscoped it is the plain ``/api`` URL, byte for byte what the
 * facility's own page has always called.
 */
export const apiScopedUrl = (facility, path) => {
    if (!facility?.id || !facility?.vertical) return `/api${path}`;
    ...
    if (facility.kind === 'branch') {
        return `/api/clinic/branches/${facility.id}/act${path}`;
    }
    return `/api/admin/operations/${facility.vertical}-members/${facility.id}/act${path}`;
};
