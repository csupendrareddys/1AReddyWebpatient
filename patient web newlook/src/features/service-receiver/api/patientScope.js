 ...
 *
 * Backend counterpart: ``/api/admin/operations/patients/<id>/act/<path>``
...
const actBase = (scope) => {
    ...
    if (typeof scope === 'string' && scope.startsWith(STAFF_FAMILY_PREFIX)) {
        return `/api/patient-staff/act-minor/${scope.slice(STAFF_FAMILY_PREFIX.length)}`;
    }
    if (typeof scope === 'string' && scope.startsWith(FAMILY_PREFIX)) {
        return `/api/patient/family/${scope.slice(FAMILY_PREFIX.length)}/act`;
    }
    if (typeof scope === 'string' && scope.startsWith(STAFF_PREFIX)) {
        return `/api/patient-staff/act/${scope.slice(STAFF_PREFIX.length)}`;
    }
    return `/api/admin/operations/patients/${scope}/act`;
};

/**
 * Build the URL for a patient self-service ``path`` (leading slash, relative
 * to ``/api/patient``) in the given scope — a guardian acting on their minor,
...
export const patientScopedUrl = (scope, path) => {
    ...
    return base ? `${base}/patient${path}` : `/api/patient${path}`;
};
