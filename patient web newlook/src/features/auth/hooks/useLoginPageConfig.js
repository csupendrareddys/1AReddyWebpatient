...
import resolveTenantSlug from '../../../common/hooks/useTenantSlug';
...
export const useLoginPageConfig = (pageType, lang = 'en') => {
    ...
    const fetchConfig = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            // Resolve tenant from hostname / VITE_TENANT_SLUG / ?tenant= so
            // e.g. ``acme.example.com`` or ``?tenant=acme`` lands on acme's
            // branded login page.
            const tenantSlug = resolveTenantSlug();
            const response = await fetchPublicPageConfig(pageType, lang, tenantSlug);
