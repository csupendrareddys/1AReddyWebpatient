...
export default function SocketManager() {
    ...
    useEffect(() => {
        ...
        // An in-app notification landed → bump the bell, and refetch the
        // domain queries its ``kind`` names so the page under the user's
        // feet updates without a refresh (e.g. a doctor accepting an
        // appointment live-refreshes the patient's appointment list).
        const KIND_TAGS = {
            appointment: ['Appointment'],
            prescription: ['PatientPrescriptions', 'Prescription'],
            order: ['MarketplaceOrder', 'Appointment'],
            payout: ['Payout', 'OrderPayout'],
            doctor_verification: ['Doctor'],
            approvals: ['Admin', 'Doctor'],
            // Apex reseller console — a child signing up on the public
            // storefront live-refreshes the tenants table + quota card.
            reseller: ['ResellerTenant', 'ResellerQuota'],
            // Seller<->tenant support chat: a reply from either side
            // live-refreshes the open thread and the seller inbox.
            support: ['SupportThread', 'SupportInbox'],
            // Seller -> tenant events (plan changed, terms resynced,
            // billing notices): refresh the tenant's own subscription
            // view so an open My Subscription page updates live.
            subscription: ['MyPlan', 'TenantSubscription'],
        };
        const onNotification = (n) => {
            invalidate([
                { type: 'Notifications', id: 'LIST' },
                ...(KIND_TAGS[n?.data?.kind] || []),
            ]);
        };
