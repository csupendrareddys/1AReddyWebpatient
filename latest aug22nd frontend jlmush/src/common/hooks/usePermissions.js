...
const usePermissions = () => {
    ...
    // Apex reseller (may operate child tenants + author its own SaaS
    // plans). A policy ENTITLEMENT from /auth/me, not a feature path —
    // backend's not_apex_tenant 403 is the boundary; this is UX gating.
    const isApexReseller = !!user?.tenant_context?.reseller?.is_apex;
