...
import SellerOnlyRoute from './common/components/VendorOnlyRoute/SellerOnlyRoute';
import TenantOnlyRoute from './common/components/VendorOnlyRoute/TenantOnlyRoute';
...
const ProductCategories = lazy(() => import('./features/admin/AdminProducts/pages/ProductCategories/ProductCategories'));
...
import RequirePlatformModule from './common/components/RequirePlatformModule/RequirePlatformModule';
...
const TenantSupportPage = lazy(() => import('./features/admin/Support/pages/TenantSupportPage'));
const SellerSupportInbox = lazy(() => import('./features/admin/Support/pages/SellerSupportInbox'));
const ResellerTenantsPage = lazy(() => import('./features/admin/Reseller/pages/ResellerTenantsPage'));
const ResellerPlansPage = lazy(() => import('./features/admin/Reseller/pages/ResellerPlansPage'));
const ResellerDnsPage = lazy(() => import('./features/admin/Reseller/pages/ResellerDnsPage'));
...
const TenantOnboarding = lazy(() => import('./features/admin/TenantDomain/pages/Onboarding'));
const TenantDomainSettings = lazy(() => import('./features/admin/TenantDomain/pages/DomainSettings'));
const TenantBillingPage = lazy(() => import('./features/admin/Billing/pages/BillingPage'));
const TenantGatewaySettings = lazy(() => import('./features/admin/GatewaySettings/pages/GatewaySettings'));
    ...
    return (
        ...
        <Routes>
            ...
            <Route path="/module/:moduleSlug" element={
                <TenantOnlyRoute><ModulePage /></TenantOnlyRoute>
            } />
            <Route path="/service/:serviceSlug" element={
                <TenantOnlyRoute><ServiceDetailPage /></TenantOnlyRoute>
            } />
            ...
            <Route path="/gallery/videos" element={
                <TenantOnlyRoute><VideoGalleryPage /></TenantOnlyRoute>
            } />
            {/* SaaS pricing page — sells tenant subdomains (clinics
                running their own larazen install). Separate from the
                marketplace "Join Our Network" flow below. SellerOnly:
                the vendor apex AND apex resellers' storefronts (P3). */}
            <Route path="/pricing/:categoryCode" element={
                <SellerOnlyRoute><SaasPricingPage /></SellerOnlyRoute>
            } />
            ...
            <Route path="/join" element={
                <TenantOnlyRoute><JoinNetworkPage /></TenantOnlyRoute>
            } />
            <Route path="/join/:vertical" element={
                <TenantOnlyRoute><RedirectToJoinWithVertical /></TenantOnlyRoute>
            } />
            ...
            <Route path="/join_receiver" element={
                <TenantOnlyRoute><JoinReceiverPage /></TenantOnlyRoute>
            } />
            ...
            <Route path="/book/:consultationType" element={
                <TenantOnlyRoute><PublicDoctorListPage /></TenantOnlyRoute>
            } />
            <Route path="/book/:consultationType/doctor/:doctorId" element={
                <TenantOnlyRoute><PublicDoctorSlotsPage /></TenantOnlyRoute>
            } />
            <Route path="/book/confirmation" element={
                <TenantOnlyRoute><BookingConfirmationPage /></TenantOnlyRoute>
            } />
            <Route path="/book/first-login" element={
                <TenantOnlyRoute><FirstLoginOtpPage /></TenantOnlyRoute>
            } />
            <Route path="/book/set-password" element={
                <TenantOnlyRoute><SetPasswordPage /></TenantOnlyRoute>
            } />

            {/* Public self-serve tenant signup (from landing-page pricing
                CTA). SellerOnly — reachable on the vendor apex and on
                apex resellers' storefronts; the backend refuses every
                other host with 404 signup_not_available. */}
            <Route path="/signup/tenant" element={
                <SellerOnlyRoute><TenantSignup /></SellerOnlyRoute>
            } />
            ...
            <Route path="/dashboard/admin" element={
                ...
                {/* A tenant's own setup + routing. Deliberately under
                    /dashboard/admin (the tenant admin tree) and NOT
                    /dashboard/platform — these act on the caller's own
                    tenant, resolved server-side from the request. */}
                <Route path="getting-started" element={<TenantOnboarding />} />
                <Route path="domain" element={<TenantDomainSettings />} />
                {/* Phase 5: the tenant pays for its own SaaS subscription,
                    and connects its OWN payment/SMS accounts. Role-gated
                    only — a suspended tenant has no features but must
                    still reach these two pages to pay its way back. */}
                <Route path="billing" element={<TenantBillingPage />} />
                <Route path="payment-gateway" element={<TenantGatewaySettings />} />
                ...
                {/* Seller support thread — reachable while SUSPENDED/INACTIVE
                    (role+RBAC gated only, never plan-gated). */}
                ...
                {/* Apex reseller console — an apex-plan tenant operating its
                    child tenants and authoring its own SaaS plans. No
                    FeatureGuard: reselling is a policy entitlement, not a
                    feature path; the backend answers 403 not_apex_tenant and
                    the pages render a "not in your plan" empty state. */}
                <Route path="reseller" element={<Navigate to="tenants" replace />} />
                <Route path="reseller/tenants" element={<ResellerTenantsPage />} />
                <Route path="reseller/plans" element={<ResellerPlansPage />} />
                <Route path="reseller/dns" element={<ResellerDnsPage />} />
                <Route path="reseller/support" element={<SellerSupportInbox scope="reseller" />} />
            </Route>

            {/* Platform Owner Dashboard — cross-tenant administration.
                allowPlatformStaff admits the VENDOR tenant's own
                super/sub-admins; each console API re-checks their RBAC
                permission server-side, so this is presentation only. */}
            <Route path="/dashboard/platform" element={
                <ProtectedRoute allowedRoles={['platform_owner']} allowPlatformStaff>
                    <AdminLayout />
                </ProtectedRoute>
            }>
                <Route index element={<Navigate to="tenants" replace />} />
                <Route path="tenants" element={<RequirePlatformModule modules="tenant_management" label="Tenants"><PlatformTenantsList /></RequirePlatformModule>} />
                <Route path="tenants/:tenantId/admins" element={<RequirePlatformModule modules="tenant_management" label="Tenant admins"><PlatformTenantAdmins /></RequirePlatformModule>} />
                <Route path="tenants/:tenantId/permissions" element={<RequirePlatformModule modules="tenant_permissions" label="Tenant permissions"><PlatformTenantPermissions /></RequirePlatformModule>} />
                ...
                <Route path="tenants/:tenantId/entitlements" element={<RequirePlatformModule modules={['plan_subscription', 'tenant_permissions']} label="Tenant entitlements"><PlatformTenantEntitlements /></RequirePlatformModule>} />
                <Route path="plans" element={<RequirePlatformModule modules="plan_catalog" label="Plans & Pricing"><PlansAdmin /></RequirePlatformModule>} />
                <Route path="support" element={<RequirePlatformModule modules="support_chat" label="the Support inbox"><SellerSupportInbox scope="platform" /></RequirePlatformModule>} />
                <Route path="addons" element={<RequirePlatformModule modules="addon_catalog" label="Add-ons"><AddonsAdmin /></RequirePlatformModule>} />
                ...
                <Route path="subscriptions" element={<RequirePlatformModule modules="plan_subscription" label="SaaS Subscriptions"><SaasSubscriptionsAdmin /></RequirePlatformModule>} />
                ...
                <Route path="landing-config" element={<RequirePlatformModule modules={['landing_config', 'landing_hero']} label="Platform Landing"><LandingConfigEditor mode="platform" /></RequirePlatformModule>} />
                <Route path="landing-config/modules/:moduleId" element={<RequirePlatformModule modules={['landing_config', 'landing_hero', 'landing_module']} label="Platform Landing"><ModuleConfigEditor /></RequirePlatformModule>} />
                <Route path="landing-config/modules/:moduleId/features/:slug" element={<RequirePlatformModule modules={['landing_config', 'landing_hero', 'landing_module']} label="Platform Landing"><FeatureConfigEditor /></RequirePlatformModule>} />
