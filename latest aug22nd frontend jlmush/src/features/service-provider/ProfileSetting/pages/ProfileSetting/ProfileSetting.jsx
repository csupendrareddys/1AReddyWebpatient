...
const ProfileSetting = ({
    ...
    return (
        <Box className="doctor-profile-container" sx={previewMode ? { opacity: 0.95, position: 'relative' } : {}}>
            ...
            {/* Danger zone — only when the doctor is acting on their OWN
                profile. Ops / clinic-link scopes must never see a "delete
                my account" that would target the OPERATOR's login. */}
            {!scopeKind && (
                <Container maxWidth="lg">
                    <DeleteAccountSection loginPath="/auth/service-provider/login" />
                </Container>
            )}
