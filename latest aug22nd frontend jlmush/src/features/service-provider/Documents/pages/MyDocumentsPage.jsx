...
const MyDocumentsPage = () => {
    ...
    // Tab 1: drafts
    const { data: draftData, isLoading: draftLoading } = useGetMyDoctorDocumentsQuery(
        { status: 'draft', page: page + 1, per_page: rowsPerPage },
        { skip: tab !== 1 },
    );

    // Tab 2: awaiting admin approval
    const { data: awaitingData, isLoading: awaitingLoading } = useGetMyDoctorDocumentsQuery(
        { status: 'pending_approval', page: page + 1, per_page: rowsPerPage },
        { skip: tab !== 2 },
    );

    // Tab 3: approved by admin (ready to push to patient)
    const { data: approvedData, isLoading: approvedLoading } = useGetMyDoctorDocumentsQuery(
        { status: 'approved', page: page + 1, per_page: rowsPerPage },
        { skip: tab !== 3 },
    );

    // Tab 4: completed (active — pushed to patient)
    const { data: completedData, isLoading: completedLoading } = useGetMyDoctorDocumentsQuery(
        { status: 'active', page: page + 1, per_page: rowsPerPage },
        { skip: tab !== 4 },
    );

    // Tab 5: rejected
    const { data: rejectedData, isLoading: rejectedLoading } = useGetMyDoctorDocumentsQuery(
        { status: 'rejected', page: page + 1, per_page: rowsPerPage },
        { skip: tab !== 5 },
    );

    // Tab 6: revised (superseded)
    const { data: revisedData, isLoading: revisedLoading } = useGetMyDoctorDocumentsQuery(
