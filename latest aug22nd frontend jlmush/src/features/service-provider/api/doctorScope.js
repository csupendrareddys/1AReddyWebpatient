 ...
 *
 *   * ``ops``  → ``/api/v1/admin/operations/doctor-members/<id>/act/<path>``
 *   * ``link`` → ``/api/v1/facility/link/doctors/<id>/act/<path>``
 *
 ...
 *    could re-assert it — one request per mount escaped to ``/api/v1/doctor/...``
...
/** ``/api/v1`` prefix each proxy hangs its allowlisted paths off. */
const SCOPE_PREFIX = {
    [OPS_SCOPE]: (id) => `/api/v1/admin/operations/doctor-members/${id}/act`,
    [LINK_SCOPE]: (id) => `/api/v1/facility/link/doctors/${id}/act`,
};
 ...
 * calling ``/api/v1/doctor/...`` as the facility and 403-ing. Closing the dialog
...
/**
 * ``/api/v1/doctor``-relative path in whatever scope is active. Every axios call
