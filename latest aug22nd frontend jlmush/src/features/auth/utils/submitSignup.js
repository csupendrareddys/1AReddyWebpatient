...
export async function submitSignupWithTokens({
    ...
    if (signupType === 'clinic') {
        try {
            ...
            await axiosInstance.post('/api/v1/auth/signup/clinic', fd, {
    ...
    if (signupType === 'hospital') {
        try {
            ...
            await axiosInstance.post('/api/v1/auth/signup/hospital', fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            return { ok: true };
        } catch (err) {
    ...
    if (signupType === 'doctor') {
        try {
            ...
            await axiosInstance.post('/api/v1/auth/signup/doctor', fd, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            return { ok: true };
        } catch (err) {
