/**
 * AdminProfileConfigLanding — Sub-module landing page for Admin (Sub-Admin) Profile controls.
 * Super admins use this to configure the sub-admin profile page.
 */
import {
...
const ADMIN_PROFILE_SUBMODULES = {
    profile_details: {
    ...
    scheduling: {
        title: 'Scheduling & Availability Module (Placeholder)',
        buttons: [
    ...
    analytics_attendance: {
        title: 'Analytics & Attendance Module (Placeholder)',
        buttons: [
...
const AdminProfileConfigLanding = () => {
    const navigate = useNavigate();

    const handleNavigate = (section) => {
        if (!section) return;
        if (section === 'preview') {
            navigate('/dashboard/admin/admin-profile-config/editor?tab=preview');
            return;
        }
        if (section === 'history') {
            navigate('/dashboard/admin/admin-profile-config/editor?tab=history');
            return;
        }
        navigate(`/dashboard/admin/admin-profile-config/editor?section=${section}`);
    };

    return (
        <Box>
            <Typography variant="h5" fontWeight={600} sx={{ mb: 2 }}>
                Sub-Admin Profile Page Controls
            </Typography>
            ...
            {Object.entries(ADMIN_PROFILE_SUBMODULES).map(([moduleKey, module]) => (
...
export default AdminProfileConfigLanding;
