...
import { flattenFeatures } from '../../../Subscription/hooks/useMySubscription';
...
/**
 * CurrentAccessTab — what this tenant ACTUALLY resolves to right now:
 * the subscription snapshot, then add-ons, then manual overrides. The
 * seller-side twin of the tenant's own My Subscription matrix. With
 * grandfathering, this is deliberately NOT the plan's current catalog
 * terms — the Source column says where each entitlement came from.
 */
const ROLE_ROWS = ['total', 'super_admin', 'sub_admin', 'provider'];

const CurrentAccessTab = ({ tenantId }) => {
    const q = useGetTenantEntitlementsQuery(tenantId);
    ...
    if (q.error) {
        return (
            <Alert severity={q.error?.status === 404 ? 'warning' : 'error'}>
                {q.error?.data?.error || 'Failed to load resolved access.'}
            </Alert>
        );
    }
    const d = q.data || {};
    const featureRows = flattenFeatures(d.features || {});
    const counts = d.counts || {};
    const limits = d.limits || {};
    const limitSources = d.limit_sources || {};
    const featureSources = d.feature_sources || {};

    return (
        <Box>
            <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1.5 }} flexWrap="wrap">
                <Chip size="small" color="primary" label={d.plan_code || '—'} />
                <Chip
                    size="small" variant="outlined"
                    color={subscriptionStatusColor[d.subscription_status] || 'default'}
                    label={d.subscription_status}
                />
                {d.is_apex && (
                    <Tooltip title="Apex reseller — sells its own plans and operates child tenants under this account">
                        <Chip size="small" color="secondary" label="APEX" />
                    </Tooltip>
                )}
                {(d.active_addons || []).map((code) => (
                    <Chip key={code} size="small" variant="outlined" label={`add-on: ${code}`} />
                ))}
            </Stack>
            <Alert severity="info" sx={{ mb: 2 }}>
                This is what the tenant resolves to <b>right now</b> — the terms
                snapshotted when the subscription was granted, plus add-ons and
                manual overrides. Catalog edits to the plan do <b>not</b> appear
                here unless you push them via &quot;Resync subscribers&quot; on
                the plan.
            </Alert>

            <Typography variant="subtitle2" sx={{ mb: 1 }}>Seats</Typography>
            <TableContainer component={Paper} variant="outlined" sx={{ mb: 3 }}>
                <Table size="small">
                    ...
                    <TableBody>
                        {ROLE_ROWS.filter((r) => limits[r] !== undefined).map((r) => (
                            <TableRow key={r}>
                                <TableCell><code>{r}</code></TableCell>
                                <TableCell align="right">{counts[r] ?? '—'}</TableCell>
                                <TableCell align="right">{limits[r]}</TableCell>
                                <TableCell>{(limitSources[r] || []).join(', ') || 'plan'}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            ...
            <Typography variant="subtitle2" sx={{ mb: 1 }}>
                Features ({featureRows.filter((f) => f.enabled).length} of {featureRows.length} enabled)
            </Typography>
                    ...
                    <TableBody>
                        {featureRows.map((f) => (
                            <TableRow key={f.path} hover>
                                ...
                                <TableCell>{featureSources[f.path] || 'plan'}</TableCell>
                                <TableCell>
...
/**
 * BuyerCard — who actually bought this tenancy. The earliest
 * super-admin (creation order) is the account that came out of the
 * signup funnel, so their name/phone/email are the seller's contact
 * for the organisation — the vendor-side equivalent of the member
 * directories tenants have for their own people.
 */
const BuyerCard = ({ tenantId, tenant }) => {
    const navigate = useNavigate();
    const adminsQ = useListTenantAdminsQuery({ tenantId });
    const admins = adminsQ.data || [];
    const supers = admins.filter((a) => a.role === 'super_admin');
    const buyer = supers[0] || admins[0] || null;
    const others = Math.max(admins.length - (buyer ? 1 : 0), 0);
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
            ...
            >
                <Box sx={{ minWidth: 220 }}>
                    ...
                    {tenant?.created_at && (
                        <Typography variant="caption" color="text.secondary">
                            Customer since {tenant.created_at.slice(0, 10)}
                        </Typography>
                    )}
                </Box>
                <Box sx={{ flex: 1 }}>
                    ...
                    {buyer && (
                        <>
                            ...
                            <Typography variant="body2">
                                ...
                                {buyer.email ? ` · ${buyer.email}` : ' · no email'}
                            </Typography>
                            <Typography variant="caption" color="text.secondary">
                                Admin since {buyer.created_at ? buyer.created_at.slice(0, 10) : '—'}
                                {buyer.last_login
                                    ? ` · last sign-in ${buyer.last_login.slice(0, 10)}` : ''}
                            </Typography>
                        </>
                    )}
                </Box>
                <Stack justifyContent="center" sx={{ minWidth: 150 }}>
                    <Button
                        size="small" variant="outlined"
                        onClick={() => navigate(
                            `/dashboard/platform/tenants/${tenantId}/admins`)}
                    >
                        {others > 0
                            ? `All admins (${admins.length})` : 'Manage admins'}
                    </Button>
                </Stack>
            </Stack>
        </Paper>
...
const TenantEntitlements = () => {
    ...
    return (
        <Container maxWidth="lg" sx={{ py: 3 }}>
            ...
            <BuyerCard tenantId={tenantId} tenant={tenantQ.data} />
            ...
            <Paper>
                ...
                <Box sx={{ p: 2 }}>
                    ...
                    {tab === 1 && <CurrentAccessTab tenantId={tenantId} />}
                    {tab === 2 && <AddonsTab tenantId={tenantId} />}
                    {tab === 3 && <TenantPermissionsMatrix tenantId={tenantId} />}
                </Box>
            </Paper>
        </Container>
    );
