...
import { useGetSymptomsMasterQuery } from '../../../api/doctorProfileConfigEndpoints';
...
const EMPTY_FORM = {
    ...
    experience_rule: [], logo_asset_id: null, targeting: null,
...
// Product categories live on their own full page (like group offerings), so
// this tab is a shortcut to that route rather than a panel on this page.
const CATEGORIES_TAB = 3;
const PRODUCT_CATEGORIES_ROUTE = '/dashboard/admin/product-categories';

const AdminProducts = () => {
    const navigate = useNavigate();
    const { data: products = [], isLoading } = useGetAdminProductsQuery();
    // Option sources for the targeting section: categories, the symptoms
    // master (the same one the doctor portal's symptoms page uses), and
    // the doctor list for the "Recommended for you" picks.
    const { data: productCategories = [] } = useGetProductCategoriesQuery();
    const { data: symptomsMaster } = useGetSymptomsMasterQuery();
    const { data: doctorsData } = useGetDoctorsQuery({ page: 1, per_page: 1000 });
    ...
    const handleTabChange = (_, value) => {
        ...
        if (value === CATEGORIES_TAB) {
            navigate(PRODUCT_CATEGORIES_ROUTE);
            return;
        }
        setTab(value);
    };
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="lg" fullWidth
                ...
                <DialogContent>
                    <Stack spacing={2} mt={1}>
                        ...
                        {/* Audience targeting — edits stay in form state and
                            ride the dialog's single Save (one API call). */}
                        <Divider textAlign="left">
                            <Typography variant="overline">Targeting — who sees this first</Typography>
                        </Divider>
                        <TargetingSection
                            value={form.targeting}
                            onChange={(t) => setForm((f) => ({ ...f, targeting: t }))}
                            categories={productCategories}
                            symptomOptions={symptomsMaster?.symptoms || []}
                            showRecommended
                            recommendedOptions={{
                                doctors: (doctorsData?.doctors || []).map((d) => ({
                                    id: String(d.doctor_id || d.id),
                                    name: `${d.first_name || ''} ${d.last_name || ''}`.trim() || d.email,
                                })),
                                specializations: specializations.map((s) => ({
                                    id: specId(s), name: s.name,
                                })),
                                // Any catalog product except the one being edited.
                                products: products
