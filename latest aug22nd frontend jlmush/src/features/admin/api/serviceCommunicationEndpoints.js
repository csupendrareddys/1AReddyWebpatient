...
const BASE = '/api/v1/service-communication';
...
const serviceCommunicationEndpoints = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getHeldVendors: builder.query({
            query: () => ({ url: '/api/v1/admin/holding-channels', method: 'GET' }),
        ...
        openHoldingChannel: builder.mutation({
            ...
            query: ({ doctor_id, user_id }) => ({
                url: doctor_id
                    ? `/api/v1/admin/holding-channels/${doctor_id}/open`
                    : `/api/v1/admin/holding-channels/user/${user_id}/open`,
                method: 'POST',
            }),
