...
import { ContactChangeControl, ChangePasswordControl } from './ContactSecurityControls';
...
const AdminPersonalProfessionalSection = React.memo(({ previewMode = false, configOverride = null, registerSave }) => {
    ...
    // Sign-in identity changed through the OTP dialogs — shown over the
    // profile payload until the next full profile fetch.
    const [contactOverride, setContactOverride] = React.useState({});
                ...
                <Grid item xs={12} md={9}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        {cfg.isFieldVisible('phone') && <Grid item xs={12} sm={6}>
                            <TextField
                                fullWidth label={cfg.getFieldLabel('phone', 'Phone Number')} name="phone_number"
                                value={contactOverride.phone_number ?? formData.phone_number} disabled
                                helperText="Sign-in number — verified by SMS code on change"
                                InputProps={{
                                    endAdornment: !previewMode && (
                                        <ContactChangeControl
                                            channel="phone"
                                            onChanged={(v) => setContactOverride((o) => ({ ...o, phone_number: v }))}
                                        />
                                    ),
                                }}
                            />
                        ...
                        {cfg.isFieldVisible('email') && <Grid item xs={12} sm={6}>
                            <TextField
                                fullWidth label={cfg.getFieldLabel('email', 'Email')} name="email"
                                value={contactOverride.email ?? formData.email} disabled
                                helperText="Sign-in email — verified by emailed code on change"
                                InputProps={{
                                    endAdornment: !previewMode && (
                                        <ContactChangeControl
                                            channel="email"
                                            onChanged={(v) => setContactOverride((o) => ({ ...o, email: v }))}
                                        />
                                    ),
                                }}
                            />
                        </Grid>}
                        {!previewMode && <Grid item xs={12}>
