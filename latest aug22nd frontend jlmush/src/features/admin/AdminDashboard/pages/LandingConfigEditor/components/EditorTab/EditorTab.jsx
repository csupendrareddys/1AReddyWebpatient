 ...
 */
/**
 * Native color input with LOCAL live value. A picker drag fires onChange on
 * every tick; holding the value locally means only this tiny component
 * re-renders per tick while ``onTick`` commits silently to the draft ref.
 * ``onCommit`` fires once on blur (picker closed / focus left) so controls
 * that render from the draft — the Custom preset ring — sync exactly once.
 */
function ColorField({ fieldKey, initial, onTick, onCommit, disabled }) {
    const [value, setValue] = useState(initial);
    // Only commit when the value actually moved since the last commit —
    // a focus-in/out with no change must not dirty the draft.
    ...
    return (
        <TextField
            ...
            disabled={disabled}
            sx={{ width: 100 }}
            inputProps={{ 'data-field': fieldKey }}
        />
    );
}
...
const EditorTab = ({ draft, canEdit, patchHero, mode = 'tenant' }) => {
    ...
    // and the section-visibility switches NEED it (they render from
    // the draft).
    const set = (k, v) => patchHero({ [k]: v }, { render: true });
    ...
    //      arbitrary colors.
    // Preset tiles are CONTROLLED by the draft (the selection ring reads
    // ``activePreset``), so a tile click must re-render — hence render: true.
    // It's a discrete click, so one re-render is the documented contract.
    const setPreset = (key) => {
        if (key === 'custom') {
            patchHero({ theme_preset: 'custom' }, { render: true });
            return;
        }
        const p = LANDING_THEME_PRESETS[key];
        if (!p) {
            patchHero({ theme_preset: key }, { render: true });
            return;
        }
        patchHero({
            theme_preset: key,
    ...
    //
    // SILENT on purpose: a native color input fires onChange on every tick
    // of a picker drag, and each render-bump repaints this whole tree. The
    // ColorField shows the live value from its own local state; the preset
    // ring syncs on the field's blur commit (render: true there).
                            ...
                            <Box>
                                {field.type === 'color' && (
                                    <ColorField
                                        ...
                                        fieldKey={field.key}
                                        initial={draft[field.key] || field.default}
                                        onTick={setColor(field.key)}
                                        onCommit={(v) => patchHero(
                                            { [field.key]: v }, { render: true })}
                                        disabled={!canEdit}
                                    />
                                )}
                                {field.type === 'select' && (
                                    <TextField
                                        ...
                                        onChange={(e) => {
                                            ...
                                            // Discrete click; selects can gate
                                            // other fields, so sync the tree.
