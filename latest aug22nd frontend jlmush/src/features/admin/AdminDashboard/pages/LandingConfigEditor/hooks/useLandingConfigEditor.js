...
const useLandingConfigEditor = ({ mode = 'tenant', skip = false } = {}) => {
    ...
    // total). This preserves the ref-based "no DOM thrash on every
    // keystroke" design while letting the Save Draft button enable
    // after the first edit.
    //
    // ``render: true`` additionally bumps ``patchVersion`` so
    // ``mergedDraft`` recalculates and controls whose RENDERED state
    // reads from the draft (theme preset tiles, section-visibility
    // switches, selects that gate other fields) reflect the edit
    // immediately. Callers firing at keystroke/drag frequency (text,
    // textarea, translations, color drag ticks) MUST stay silent —
    // bumping per keystroke re-renders the whole editor tree per key,
    // which is the exact regression this split exists to prevent.
    const patchHero = (updates, { render = false } = {}) => {
        Object.assign(heroPatchRef.current, updates);
        setIsDirty(true);
        if (render) setPatchVersion(v => v + 1);
    };
    const resetPatch = () => {
