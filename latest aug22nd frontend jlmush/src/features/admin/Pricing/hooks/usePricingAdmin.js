...
const INITIAL_PLAN_FORM = {
    ...
    // Reseller fields (vendor console only). kind='apex' turns the
    // subscribing tenant into a reseller; the child quotas cap how many
    // sub-tenants it may create. Backend cross-validates the trio.
    kind: 'normal',
    max_child_subdomains: null,
    max_child_custom_domains: null,
    child_plan_caps: null,
    addon_terms: null,
    card_display: null,
};
...
const buildPlanPayload = (form, isReceiver) => {
    if (isReceiver) {
        return {
            ...
            features: {},
            default_addons: [],
            // A receiver plan can never be an apex/reseller plan.
            kind: 'normal',
            max_child_subdomains: null,
            max_child_custom_domains: null,
        };
    }
    // Provider/tenant plans now ALSO carry marketing benefits — the
    // pricing card renders them verbatim when present.
    return {
        ...form,
        benefits: (form.benefits || [])
            .map((b) => (b || '').trim())
...
const INITIAL_PLAN_TYPE_FORM = {
    ...
    description: '',
    icon_key: '',
    // Industry segment (SaaS category) this type belongs to. Empty string
    // reads as "default category" server-side.
    category_id: '',
...
export const usePricingAdmin = ({ scope = 'platform' } = {}) => {
    const dispatch = useDispatch();

    // ── Server state (RTK Query — no slice needed) ────────────
    // ``scope``: 'platform' (vendor console, default) or 'reseller'
    // (apex console) — routes to the matching backend surface and keys
    // the cache tags so the two catalogs never cross-invalidate.
    const plansQuery = useListPlansQuery(scope);
    const planTypesQuery = useListPlanTypesQuery(scope);
    // The add-on catalog is the VENDOR's — the reseller surface has no
    // mirror for it (default_addons is stripped server-side there too).
    const addonsQuery = useListAddonsQuery(
        undefined, { skip: scope !== 'platform' });
    ...
    const openPlanDialog = useCallback((existing) => {
        if (existing) {
            ...
            setPlanForm({
                ...
                data_retention_days: existing.data_retention_days ?? 180,
                benefits: existing.benefits || [],
                child_plan_caps: existing.child_plan_caps || null,
                addon_terms: existing.addon_terms || null,
                card_display: existing.card_display || null,
                razorpay_supported: existing.razorpay_supported ?? true,
                kind: existing.kind || 'normal',
                max_child_subdomains: existing.child_limits?.subdomains ?? null,
                max_child_custom_domains: existing.child_limits?.custom_domains ?? null,
    ...
    }, [createPlan, updatePlan, planForm, editingPlanCode, isReceiverPlan, notify, scope]);
    ...
    const handleUpdateAddon = useCallback(
        // ``quiet`` suppresses the per-add-on toast and RE-THROWS, so a
        // bulk caller (the pricing grid) can report once and stop on the
        // first failure instead of firing a dozen snackbars.
        async (code, patch, { quiet = false } = {}) => {
            try {
                await updateAddon({ code, data: patch }).unwrap();
                if (!quiet) notify('success', `Add-on "${code}" updated`);
            } catch (err) {
                if (quiet) throw err;
                notify('error', err?.data?.error || 'Update failed');
            }
