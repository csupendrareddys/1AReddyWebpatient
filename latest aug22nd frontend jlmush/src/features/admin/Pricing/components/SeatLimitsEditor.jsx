...
// An ADD-ON can grant on three axes, not just team seats: marketplace
// entities and (for a reseller) extra child tenancies. The plan dialog
// keeps the four seat fields; the add-on dialog passes ALL_GRANT_KEYS so
// an entity or tenancy add-on can actually be authored.
export const ALL_GRANT_KEYS = [
    ...ROLE_KEYS,
    { key: 'doctor', label: 'Doctor entities' },
    { key: 'clinic', label: 'Clinic entities' },
    { key: 'hospital', label: 'Hospital entities' },
    { key: 'child_subdomain', label: 'Subdomain tenancies' },
    { key: 'child_custom_domain', label: 'Custom-domain tenancies' },
];
...
const SeatLimitsEditor = ({
    ...
    keys = ROLE_KEYS,       // ALL_GRANT_KEYS for add-ons
}) => {
    const v = value || {};
