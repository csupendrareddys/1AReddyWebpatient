...
import DeleteAccountSection from '../../../../../common/components/DeleteAccountSection/DeleteAccountSection';
...
const ProfileSetting = ({
    ...
    const {
        ...
        // unscoped ``/api/v1/patient/*`` thunks 403 with "Required roles: ['patient']"
    ...
    return (
        <Box className="profile-setting-container" sx={{
            ...
            {/* Danger zone — only when the logged-in patient is acting on
                THEMSELVES. A guardian/caregiver/ops scope must never see a
                "delete my account" that would target the OPERATOR's login. */}
            {scopeKind === 'self' && (
                <DeleteAccountSection loginPath="/auth/service-receiver/login" />
            )}
